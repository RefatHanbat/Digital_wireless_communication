...
constellation = exp(i*2*pi.*a/2);
%To store constellation points ie. 1+0j and -1+0j in complex 
% double number of symbols in simulation
nsymbols = 100000; %Change it to 10,00,000 > represents number of 
% symbols used for stimulation.
...
for k=1:number_snrs %SNR for loop
    snr_now = snr(k); %The current value of snr being tested 
    % for BER.
    ...
    % To add complex WBGN noise to our input signal -1.
    decisions=zeros(nsymbols,1); 
    %We initialize decisions with zeros corresponding to all 
    % n symbols.
    for n=1:nsymbols %Symbol for loop to compute distance 
        % which is used to stimulate recieved signal being 1 or -1.
        distances = abs(received(n)-constellation); 
        %It stores a vector of absolute distance 
                                                        %from each of the constellation points 1 and -1.
        [min_dist,decisions(n)] = min(distances); 
        %We store the minimum of those distances with memory location 
