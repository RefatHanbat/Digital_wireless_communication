...
SNRdB = -8 : 1 : 12; % set signal to noise ratio in db
...
for k = 1:length(SNR)
    ...
    % (-1) * (-1) -> (+1), not flipped 
    % (+1) * (+1) -> (+1), not flipped 
    %no of indice which are less than one(+1), because (-1) indices 
    % the bit
    %values has flipped

    noisy_bits = y.* x_BPSK; 
    % Logic to get number of corrupted bits 
    % by noise
   ...
   % NumberOfCurrupted_bits(k) =length(indices_currupted) ;
